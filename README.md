...
CLI 支持单轮查询、循环监控、变更比较及告警。GUI 支持目标管理、监听设置、手动及自动查询，并使用现有配置中的通知通道；通知配置界面尚未实现。
...
点击“停止监听”取消下一轮调度，但不强制中断当前请求。最小化后仍监听，关闭程序后停止；查询中关闭会先停止监听并提示等待本轮完成。编辑弹窗打开时暂缓触发自动查询，关闭后若倒计时已到期再执行。配置错误暂停监听，查询失败等待下一轮重试，不并行执行多轮。
